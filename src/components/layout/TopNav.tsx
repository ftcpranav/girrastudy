      ...
      <div ref={searchRef} className="relative w-full max-w-md flex items-center gap-2">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-10 pr-9 py-1.5 bg-slate-100 dark:bg-slate-900/40 border border-slate-200 dark:border-emerald-500/20 rounded-xl text-sm focus:outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/10 placeholder-slate-400 dark:placeholder-slate-500 text-slate-900 dark:text-slate-200 transition-all"
          />
          {searchQuery && (
            <button
        ...
        <button
          ...
          className="hidden sm:flex items-center gap-1 px-2.5 py-1.5 rounded-xl bg-slate-100 dark:bg-slate-900/60 hover:bg-slate-200 dark:hover:bg-slate-900 border border-slate-200 dark:border-emerald-500/20 text-slate-500 dark:text-slate-400 text-xs font-mono transition-colors cursor-pointer"
        ...
        {showResults && (
          <div className="absolute top-12 left-0 right-0 glass-card rounded-2xl p-4 shadow-2xl z-50 max-h-[80vh] overflow-y-auto bg-white dark:bg-slate-950 border border-emerald-500/20">
            {isSearching ? (
              ...
              <div className="space-y-4">
                ...
                {results.subjects.length > 0 && (
                  <div>
                    <h3 className="text-[10px] font-bold uppercase tracking-wider text-emerald-500 dark:text-emerald-400 mb-2 border-b border-emerald-500/15 pb-1">
                    ...
                    <div className="space-y-1">
                      {results.subjects.map((sub) => (
                        <button
                          ...
                          className="w-full text-left flex items-center gap-2.5 px-2.5 py-1.5 hover:bg-emerald-500/10 rounded-lg transition-colors text-slate-800 dark:text-slate-200 text-xs"
                        >
                    ...
                    <div className="space-y-1">
                      {results.assessments.map((ast) => (
                        <button
                          ...
                          className="w-full text-left flex items-center justify-between px-2.5 py-1.5 hover:bg-emerald-500/10 rounded-lg transition-colors text-xs"
                        >
                          <div className="flex items-center gap-2.5 truncate">
                            <CheckSquare className="h-3.5 w-3.5 text-amber-500 shrink-0" />
                            <span className="font-medium text-slate-800 dark:text-slate-200 truncate">{ast.name}</span>
                          </div>
                          <span className="text-[10px] text-slate-500 dark:text-slate-400 italic bg-slate-100 dark:bg-slate-900 px-1.5 py-0.5 rounded">
                            {ast.subject?.name}
                          </span>
                        </button>
                      ))}
                      ...
                      {results.notes.map((note) => (
                        ...
                        >
                          <div className="flex items-center gap-2.5 truncate">
                            ...
                            <span className="font-medium text-slate-800 dark:text-slate-200 truncate">{note.title}</span>
                          </div>
