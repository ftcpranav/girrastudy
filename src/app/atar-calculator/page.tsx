...
export default function AtarCalculatorPage() {
  ...
  return (
    <AppLayout>
      ...
      <div className="glass-card rounded-3xl p-6 border-indigo-950/20 mb-6 bg-gradient-to-r from-slate-950 via-slate-900/60 to-violet-950/30">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          <div>
            <div className="flex items-center gap-2 mb-1">
              <span className="p-1.5 rounded-xl bg-violet-600/20 text-violet-400 border border-violet-500/20">
          ...
          <div className="flex items-center gap-4 bg-slate-950/80 px-6 py-3 rounded-2xl border border-indigo-950/40 shrink-0">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mt-6 pt-4 border-t border-indigo-950/20">
          {/* Eligibility status */}
          <div>
            {validation.isValid ? (
      ...
      {validation.warnings.length > 0 && (
        <div className="mb-6 p-3 rounded-xl bg-amber-500/10 border border-amber-500/20 text-amber-400 text-xs flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="glass-card rounded-3xl p-6 border-indigo-950/20">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-5 pb-3 border-b border-indigo-950/15">
              <div>
                <h3 className="text-base font-bold text-slate-100">Selected Subjects ({selectedCourses.length})</h3>
              ...
              <div className="flex items-center gap-2">
                <select
                  ...
                  className="px-3 py-1.5 bg-slate-950 border border-indigo-950/30 rounded-xl text-xs text-slate-200 focus:outline-none focus:border-violet-500/50"
                >
                ...
                <button
                  ...
                  className="px-3 py-1.5 bg-violet-600 hover:bg-violet-500 disabled:opacity-40 text-white rounded-xl font-bold text-xs transition-all cursor-pointer flex items-center gap-1"
                >
            ...
            <div className="space-y-3">
              {selectedCourses.map((sc) => {
                ...
                return (
                  <div
                    key={sc.courseId}
                    className="p-4 bg-slate-950/40 border border-indigo-950/20 rounded-2xl transition-all hover:border-violet-500/20"
                  >
                    {/* Header Row */}
                    <div className="flex items-center justify-between gap-3 mb-3">
                      <div className="flex items-center gap-2.5 min-w-0">
                        <span className="px-2 py-0.5 rounded-md bg-violet-600/15 border border-violet-500/25 text-violet-400 font-mono font-bold text-xs">
                          {course.units}U
                        </span>
                        <h4 className="text-xs font-extrabold text-slate-100 truncate">{course.name}</h4>
                      </div>

                      {/* Right stats badge */}
                      <div className="flex items-center gap-3 shrink-0">
                        <span className="text-[11px] text-slate-400 font-mono">
                          Mark: <strong className="text-violet-300 font-bold">{compositeMark}%</strong>
                        </span>
                        <span className="text-[11px] text-slate-400 font-mono">
                          Scaled: <strong className="text-emerald-400 font-bold">~{Math.round(scaledMark * 10) / 10}/50</strong>
                        </span>
                        <button
                          type="button"
                    ...
                    {/* Compact Input Row */}
                    {calculationMode === 'moderated' ? (
                      <div className="grid grid-cols-3 gap-3 pt-2 border-t border-indigo-950/15">
                        <div className="space-y-1">
                          <label className="text-[9px] font-bold text-slate-400 uppercase block">School Mark (50%)</label>
                          <input
                            type="number"
                            min="0"
                            max="100"
                            ...
                            className="w-full px-2.5 py-1 bg-slate-900 border border-indigo-950/30 rounded-lg text-xs font-mono text-slate-200 focus:outline-none focus:border-violet-500"
                          />
                        </div>

                        <div className="space-y-1">
                          <label className="text-[9px] font-bold text-slate-400 uppercase block">Exam Mark (50%)</label>
                          <input
                            type="number"
                            min="0"
                            max="100"
                        ...
                        <div className="space-y-1">
                          <label className="text-[9px] font-bold text-slate-400 uppercase block">School Rank Pct</label>
                          <input
                            type="number"
                            min="1"
                            max="100"
                      ...
                      <div className="flex items-center gap-3 pt-2 border-t border-indigo-950/15">
                        ...
                        <div className="relative shrink-0">
                          <input
                            ...
                            className="w-16 px-2 py-1 bg-slate-900 border border-indigo-950/30 rounded-lg text-xs font-mono font-bold text-center text-violet-300 focus:outline-none focus:border-violet-500"
                          />
        ...
        <div className="space-y-6">
          ...
          <div className="glass-card rounded-3xl p-5 border-indigo-950/20">
            ...
            <div className="p-3 rounded-xl bg-slate-950/50 border border-indigo-950/20 flex items-center justify-between text-xs font-mono">
              <span className="text-slate-400">Target Gap:</span>
              <span className={`font-bold ${results.atar >= targetAtar ? 'text-emerald-400' : 'text-amber-400'}`}>
          ...
          <div className="glass-card rounded-3xl p-5 border-indigo-950/20">
            <div className="flex items-center justify-between mb-4 pb-2 border-b border-indigo-950/15">
              <h3 className="text-xs font-bold text-slate-200 flex items-center gap-1.5">
              ...
              {/* Chart Tabs */}
              <div className="inline-flex p-0.5 bg-slate-950 border border-indigo-950/30 rounded-lg">
                <button
                  type="button"
